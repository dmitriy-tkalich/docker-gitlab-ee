...
If using the latest docker version and/or disabling selinux does not fix the issue then please file a issue request on the [issues](https://github.com/creativ/docker-gitlab-ee/issues) page.
...
```bash
docker pull creative/gitlab-ee:8.13.0
```

You can also pull the `latest` tag which is built from the repository *HEAD*

```bash
docker pull creative/gitlab-ee:latest
```

Alternatively you can build the image locally.

```bash
docker build -t creative/gitlab-ee github.com/creative/docker-gitlab-ee
```
...
```bash
wget https://raw.githubusercontent.com/creative/docker-gitlab-ee/master/docker-compose.yml
```
...
docker run --name gitlab -d [OPTIONS] \
    creative/gitlab-ee:8.13.0 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    creative/gitlab-ee:8.13.0 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    creative/gitlab-ee:8.13.0 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    creative/gitlab-ee:8.13.0 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    creative/gitlab-ee:8.13.0 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    creative/gitlab-ee:8.13.0 app:rake gitlab:import:repos
```
...
> **Note**
>
> Upgrading to `creative/gitlab-ee:8.13.0` from `creative/gitlab-ee:7.x.x` can cause issues. It is therefore required that you first upgrade to `creative/gitlab-ee:8.0.5-1` before upgrading to `creative/gitlab-ee:8.1.0` or higher.
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    creative/gitlab-ee:x.x.x app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -d [OPTIONS] creative/gitlab-ee:8.13.0
```
